...
def first_MR_phase(buckets, threshold, save_to, k=1):
    ...
    if len(candidates) > 0:
        if k == 1:
            save_itemset('intr_candidates',sorted(candidates),k,header="Candidates:",mode='w')
        else:
            save_itemset('intr_candidates',sorted(candidates),k,header=False,mode='a')
...
def second_MR_phase(buckets, candidates, support, k=1):
    ...
    if k == 1:
        result = buckets.filter(lambda x: x[0] in candidate_set) \
    ...
    if len(result.take(1)) > 0:
        if k == 1:
            save_itemset('intr_freq',sorted(result),k,header="Frequent Itemsets:",mode='w')
        else:
            save_itemset('intr_freq',sorted(result),k,header=False,mode='a')
...
def SON(buskets, support, input_file_path, output_file_path):
    ...
    while len(first) > 0:
        if k == 1:
            frequent_items = sorted(set(second))
        else:
            frequent_items = sorted({item for itemset in second for item in itemset})
        k += 1
        ...
        counts = buskets.flatMap(lambda x: [(bsn, 1) for bsn in itertools.combinations(sorted(x[1]), k) if set(bsn).issubset(frequent_items)]) \
            .reduceByKey(lambda x, y: x + y)
